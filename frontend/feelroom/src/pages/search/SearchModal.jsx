...
const SearchModal = ({
    ...
    const getRecommendedContent = () => {
        const content = {
            ...
            USER: {
                title: '유저의 닉네임을 검색해주세요',
                keywords: [

                ]
            }
        };
        return content[activeTab] || content.MOVIE;
    };
    ...
    const renderTabContent = () => {
        if (!searchTerm) {
            const emptyContent = {
                ...
                KEYWORD: '검색어를 입력하면 키워드 검색 결과를 볼 수 있습니다',
        ...
        if ((searchLoading && activeTab === 'MOVIE') ||
            (keywordSearchLoading && activeTab === 'KEYWORD') ||
        ...
        if ((searchError && activeTab === 'MOVIE') ||
            (keywordSearchError && activeTab === 'KEYWORD') ||
            ...
            return (
                <div style={{
                    ...
                    <div style={{ color: '#dc3545', fontSize: '1.1rem' }}>
                        에러: {searchError || keywordSearchError || userError}
                    </div>
                </div>
            );
        }
        ...
        switch (activeTab) {
            ...
            case 'KEYWORD':
                if (keywordSearchResults.length === 0 && !keywordSearchLoading) {
                    return (
                        <div style={{
                            display: 'flex',
                            flexDirection: 'column',
                            ...
                            <div style={{ fontSize: '1.1rem', color: '#666' }}>
                                "{searchTerm}"에 대한 키워드 검색 결과가 없습니다.
                            </div>
                            <div style={{ fontSize: '0.9rem', color: '#888' }}>
                                다른 키워드를 시도해보세요.
                            </div>
                ...
                // 키워드 검색도 MovieSelection과 완전히 동일한 그리드 스타일 사용
                return (
                    <div
                        style={{
                            display: 'grid',
                    ...
                    >
                        {keywordSearchResults.map((movie, index) => (
                            <div
                                key={movie.movie_id}
                                role="button"
                                tabIndex={0}
                ...
                {searchTerm ? (
                    <div>
                        <p style={{
                            ...
                            {activeTab === 'KEYWORD' && keywordSearchResults.length > 0 &&
                                ` (${keywordSearchResults.length}건)`
                            }
                    ...
                    <div>
                        <p style={{
                            ...
                            {getRecommendedContent().title}
                        </p>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            {getRecommendedContent().keywords.length > 0 ? (
                                getRecommendedContent().keywords.map((keyword, idx) => (
                                    <div
                                        key={idx}
                                        style={{
                                            padding: '0.75rem',
                                ...
                                <div style={{
                                    ...
                                    {activeTab === 'MOVIE' ? '추천 검색어를 불러오는 중...' :
                                        activeTab === 'KEYWORD' ? '추천 키워드가 준비되어 있습니다.' :
                                            '정확한 닉네임을 입력해주세요'}
                                </div>
                            )}
                        </div>
